...
server = app.server
app.title = "Dashboard de Casos de SRAG (Vigilância)"
...
try:
    ...
    logger.info("Dados SRAG carregados: %s linhas | %s RMs | %s anos", len(df), len(rms), len(anos))
except Exception as e:
    logger.error("Erro ao inicializar dados SRAG: %s", e)
    df = pd.DataFrame()
    rms = []
    anos = []
    thresholds_by_rm = {}
            ...
            [
                dbc.Col(
                    [
                        ...
                        html.H2(
                            "Casos de SRAG - Vigilância",
                            className="text-center my-4",
                        ),
                        html.P(
                            "Série temporal mensal de casos de SRAG nas Regiões Metropolitanas (vigilância SRAG), com limiares por RM.",
                                ...
                                [
                                    dbc.Row(
                                        [
                                            dbc.Col(
                                                [
                                                    html.Div("RM", className="control-label"),
                                                    dcc.Dropdown(
                                                        id="rm",
                                            ...
                                            dbc.Col(
                                                [
                                                    ...
                                                    dcc.RangeSlider(
                                                        id="anos",
                                                        min=min(anos) if anos else 2019,
                                                        max=max(anos) if anos else 2024,
                                                        step=1,
                                                        value=[min(anos), max(anos)] if anos else [2019, 2024],
                                                        marks=None,
        ...
        dbc.Row(
            [
                dbc.Col(
                    dbc.Card(
                        [
                            dbc.CardHeader(html.H5("Série temporal de casos", className="mb-0 text-center")),
                            dbc.CardBody(
                    ...
                    [
                        dbc.Alert(
                            [
                                html.B("Dados: "),
                                html.Span(
                                    "coloque `RM_banco_SRAG.RDATA` em `SAUDE_SRAG/data/` "
                                    "ou gere `processed/srag_monthly.parquet` com `prepare_data.py`."
                                ),
                            ],
                            id="data-alert",
                            color="warning",
...
def update_plot(rm, anos_range, show_limiar):
    if df.empty or not rm or not anos_range:
        return _empty_figure(
            "Dados indisponíveis. Verifique `SAUDE_SRAG/data/RM_banco_SRAG.RDATA` ou o parquet em `processed/`."
        )

    y0, y1 = int(anos_range[0]), int(anos_range[1])
    dff = df[(df["RM"] == rm) & (df["ano"] >= y0) & (df["ano"] <= y1)].copy()
    if dff.empty:
    ...
    fig.update_layout(
        ...
        title=f"{rm} — Série temporal mensal (casos SRAG)",
        xaxis_title="Mês/Ano",
